class scarper:
	...
	# AUN EN PRUEBAS NO ES ESTABLE!!!!!!!!!!!!!!!!!!!
	def obtenerExperiencia:
		...
		for i, entrada in enumerate(arreglo_sin_fecha, start=1):
			...
			patron = r'(ENE|FEB|MAR|ABR|MAY|JUN|JUL|AGO|SEP|OCT|NOV|DIC)[.,]?\s.*'  #MEJORABLE
			
			if re.findall(patron, entrada):
				
				arreglo_sin_fecha.insert(i, "------------") # detalle con las descripciones
